...
## Product V2 document decisions

- Timeline remains primary; bottom navigation stays Trip / + / Account.
- Booking Detail is the primary place for tickets and documents.
- The Trip header provides one secondary Tickets & Documents collection grouped by booking in journey order.
- Never introduce a primary Documents tab or file-manager/folder model.
- Show truthful per-document states: Available offline, Available on this device, Stored on this device, or Not available offline.
- Keep uncertain files under Needs Attention and require explicit Link to Booking rather than guessing.
- Removing a document never removes its booking.
