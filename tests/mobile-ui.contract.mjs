...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('tripto-shell-product-v2-welcome-route-matrix-v10'),'clean route, canonical host, lazy search, manual-attachment, booking-email inbox, live-flight, or Google-auth shell cache contract changed');
...
assert(css.includes('background:var(--accent);color:var(--card)')&&css.includes('.welcome-route-matrix')&&css.includes('.welcome-route-cell--next')&&!css.includes('--welcome-serif'),'Welcome must use the selected Route Matrix visual and system font');
...
assert(css.includes('.first-run-screen.welcome-thread')&&css.includes('height:100svh')&&css.includes('--welcome-title:clamp(38px,min(11vw,6.2svh),52px)')&&css.includes('.welcome-route-matrix')&&css.includes('@media(max-height:620px)')&&css.includes('max-width:400px'),'approved compact height-aware Welcome visual system missing');
