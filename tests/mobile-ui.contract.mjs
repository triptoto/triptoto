...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v59-trip-options-route'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
assert(app.includes('const ICON_ALIAS = Object.freeze({')&&app.includes('const ICON_SPRITE = "/icons/tripto-system.svg?v=phosphor-v4"')&&app.includes('const FILLED_ICON_IDS = new Set(')&&app.includes('<use href="${ICON_SPRITE}#${symbol}"></use>'),'local Phosphor sprite resolver missing');
