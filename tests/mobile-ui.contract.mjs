...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v97-place-retry'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
for(const copy of ['Your trip is taking shape.','Find a flight','Find a stay','Travel eSIM','Search flights','Search stays','Search with Aviasales.','Search Booking.com with Stay22.','Partner links may earn Tripto a commission at no extra cost to you.','Provided by Tripto from the Travel eSIM page.','No booking is made on this screen.'])assert(tripSetup.includes(copy),`post-dates trip setup missing: ${copy}`);
assert(app.includes('state.sheet = "trip-setup-ready"')&&app.includes('state.afterTripCreateScreen = target.dataset.afterCreate === "esim" ? "esim" : null')&&app.includes('data-action="complete-trip-setup-partner"')&&app.includes('state.afterTripCreatePartner = partnerUrl')&&app.includes('location.assign(partnerAfterTrip)')&&app.includes('requestAnimationFrame(() => document.getElementById("native-form")?.requestSubmit())'),'post-dates trip setup must save the trip before opening Travel eSIM or a partner search');
assert(app.includes('marker=465464')&&app.includes('trs=252129')&&app.includes('lmaID: STAY22_LMA_ID')&&app.includes('https://scripts.stay22.com/letmeallez.js'),'approved Aviasales and Stay22 partner configuration missing');
for(const selector of ['.trip-setup-ready','.trip-setup-ready__hero','.trip-setup-tool--flight','.trip-setup-tool--stay','.trip-setup-tool--esim','.trip-setup-tool__action'])assert(css.includes(selector),`post-dates trip setup design missing: ${selector}`);
