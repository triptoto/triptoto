...
assert(navFn.includes('navBtn("trips", "plane", "Trip")')&&navFn.includes('navBtn("trip-options", "map", "Trip options")')&&navFn.includes('data-action="open-add"')&&navFn.includes('navBtn("checklist", "checklist", "To-do")')&&navFn.includes('navBtn("account", "user", "Account")'),'V2 navigation must remain Trip / Trip options / Add / To-do / Account with the approved airplane Trip icon');
...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v58-unified-nav'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
assert(css.includes('background:color-mix(in srgb,var(--activity-soft) 72%,var(--card))')&&!css.includes('.bottom-nav .nav-item:nth-child(1) .nav-item__icon'),'bottom navigation must use one unified pastel surface without per-destination icon colors');
...
assert(app.includes('function tripOptionsScreen(')&&app.includes('case "trip-options": html = tripOptionsScreen()')&&app.includes('route("trip-options")')&&app.includes('data-screen="documents"'),'full-page trip options (weather/map/documents/edit/help) missing');
...
assert(app.includes('const ICON_ALIAS = Object.freeze({')&&app.includes('const ICON_SPRITE = "/icons/tripto-system.svg?v=phosphor-v3"')&&app.includes('const FILLED_ICON_IDS = new Set(')&&app.includes('<use href="${ICON_SPRITE}#${symbol}"></use>'),'local Phosphor sprite resolver missing');
...
for(const name of ['flight','map','notifications','checklist','traveler'])assert(iconSprite.includes(`<symbol id="${name}--fill"`),`selected-state icon missing from sprite: ${name}`);
