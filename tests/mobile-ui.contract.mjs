...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v88-elegant-pickers'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
assert(app.includes('data-place-example="Paris"')&&app.includes('trip-create-search-guide__privacy')&&app.includes('Private on this phone · ready offline')&&app.includes('example.dataset.placeExample'),'destination search must provide useful, private, keyboard-accessible example searches');
...
assert(app.includes('const ownRequest = ++request;')&&app.includes('window.setTimeout(() => search(ownRequest), 70)')&&app.includes('fullScreenPanel?.addEventListener("keydown"')&&app.includes('trip-create-search-guide__examples button:not([disabled])')&&app.includes('event.key !== "Tab"'),'Create Trip destination search must reject stale results and trap keyboard focus inside the full-screen task');
