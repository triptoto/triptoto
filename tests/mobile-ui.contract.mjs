...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v248-compact-profile'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
assert(tripsInventory.includes('class="screen trips-screen"')&&tripsInventory.includes('trips-app-bar')&&tripsInventory.includes('trip-list-row ds-flat-row')&&tripsInventory.includes('bottomNav("trips")')&&!tripsInventory.includes('trips-fab'),'Trips must use the shared app bar, flat rows, in-flow navigation, and no floating action button');
