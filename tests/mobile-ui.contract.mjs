...
assert(css.includes('--welcome-title:32px'),'Welcome headline must preserve the approved responsive display size');
...
assert(navFn.includes('data-action="open-navigation"')&&navFn.includes('data-action="open-add"')&&!app.includes('function bottomNav('),'primary navigation must use header Add and Menu without a bottom bar');
for(const label of ['All trips','Trip Options','To-Do List','Account'])assert(navFn.includes(`"${label}"`),`navigation destination missing: ${label}`);
...
// reload/back never drops the traveler onto an unavailable-detail screen.
{const start=app.indexOf('function routeEntities('),end=app.indexOf('function readableRouteId(',start),state={trips:[],collections:[{id:'collection-id',trip_item_id:'timeline-item-id',title:'Trastevere'}],transport:[],stays:[],timeline:[],travelers:[],locations:[],selectedId:null},context={state,routes:{slugify:value=>String(value).toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')},val:(object,...keys)=>{for(const key of keys)if(object?.[key]!=null&&object[key]!=='')return object[key];return null;},itemId:item=>String(item?.id||item?.trip_item_id||''),localStorage:{setItem(){}}};runInNewContext(app.slice(start,end),context);assert(context.resolveRouteId('collection','timeline-item-id')==='collection-id','collection trip-item IDs must resolve to their canonical collection record');assert(context.requiresResolvedRouteEntity('plan','missing-plan')===true&&context.requiresResolvedRouteEntity('collection-form','new:neighborhood')===false,'detail-route recovery must protect real records without blocking new forms');const missingPlan=context.missingEntityDestination('plan');assert(missingPlan.screen==='timeline'&&missingPlan.id===null,'stale plan routes must recover to the timeline');}
// Detail actions must resolve a present entity before routing. This exercises
// the actual resolver rather than merely checking the rendered action text.
{const start=app.indexOf('function detailRouteForItem('),end=app.indexOf('function openTimelineItemDetail(',start),state={timeline:[{id:'idea-1',title:'Museum'}],transport:[{id:'flight-1',transport_type:'flight'}],stays:[{id:'stay-1'}]},context={state,itemId:item=>String(item?.id||item?.trip_item_id||''),val:(object,...keys)=>{for(const key of keys)if(object?.[key]!=null&&object[key]!=='')return object[key];return null;},collectionForItem:id=>id==='collection-item'?{id:'collection-1',trip_item_id:'collection-item'}:null,transportForItem:id=>state.transport.find(item=>item.id===id)||null,stayForItem:id=>state.stays.find(item=>item.id===id)||null};runInNewContext(app.slice(start,end),context);for(const [id,screen] of [['collection-item','collection'],['flight-1','flight'],['stay-1','hotel'],['idea-1','plan']])assert(context.detailRouteForItem(id)?.screen===screen,`detail resolver chose the wrong destination for ${id}`);assert(context.detailRouteForItem('deleted-item')===null,'a deleted item must not route to a detail screen');}
for(const action of ['timeline-detail','booking-detail','notification-open'])assert(new RegExp(`case "${action}":[\\s\\S]{0,180}?openTimelineItemDetail\\(target\\.dataset\\.id\\)`).test(app),`${action} bypasses the safe detail resolver`);
// A partial load must keep prior verified sections and apply the sections that
// arrive. A temporary collections failure can therefore never blank plans.
...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v260-trip-celebration'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
const tripsInventory=app.slice(app.indexOf('function tripsPageHeader('),app.indexOf('function meaningfulBookingStatus('));
assert(tripsInventory.includes('class="screen trips-screen"')&&tripsInventory.includes('trips-app-bar')&&tripsInventory.includes('trip-list-row ds-flat-row')&&!tripsInventory.includes('bottomNav(')&&tripsInventory.includes('data-screen="account" aria-label="Account"')&&!tripsInventory.includes('trips-fab'),'Trips must use the shared app bar with Account, flat rows, and no bottom navigation or floating action button');
const createTripForm=app.slice(app.indexOf('function basicMobileForm('),app.indexOf('function zonedDateTimeParts('));
assert(createTripForm.includes('trip-create-head trip-create-intro')&&!createTripForm.includes('trip-create-route" aria-hidden'),'Create Trip must use the same direct page intro as the rest of the app instead of an isolated illustrated route hero');
assert(app.includes('mobile-v1-screen app-surface'),'PageShell routes must opt into the shared operational page surface');
...
for(const marker of ['trip-create-route','trip-create-destination__head','trip-create-details'])assert(app.includes(marker),`Create Trip travel canvas missing: ${marker}`);
...
assert(navFn.includes('aria-current="page"'),'navigation must identify the current destination');
for(const [alias,target] of [['plane','flight'],['trash','delete'],['bell','notifications'],['user','traveler'],['chevron','chevron-right'],['pin','location'],['qr','qr-code'],['document','documents'],['users','travelers'],['external','external-link'],['check-circle','confirmed'],['dest-mountain','mountain'],['dest-beach','beach'],['dest-monument','landmark']])assert(new RegExp(`["']?${alias}["']?:\\s*"${target}"`).test(app),`approved icon alias mapping missing: ${alias} -> ${target}`);
assert(css.includes('.first-run-screen.welcome-thread')&&css.includes('height:100svh')&&css.includes('--welcome-title:32px')&&css.includes('.welcome-features')&&css.includes('@media(max-height:620px)')&&css.includes('max-width:400px'),'compact height-aware Welcome visual system missing');
