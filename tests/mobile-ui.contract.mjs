...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v94-trip-setup'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
for(const copy of ['Your trip is taking shape.','Find a flight','Find a stay','Travel eSIM','Aviasales','Booking.com','Provided by Tripto from the Travel eSIM page.','No booking is made on this screen.'])assert(tripSetup.includes(copy),`post-dates trip setup missing: ${copy}`);
assert(app.includes('state.sheet = "trip-setup-ready"')&&app.includes('state.afterTripCreateScreen = target.dataset.afterCreate === "esim" ? "esim" : null')&&app.includes('requestAnimationFrame(() => document.getElementById("native-form")?.requestSubmit())'),'post-dates trip setup must create the trip and support opening the real Travel eSIM screen');
