...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v249-trips-header'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
assert(neutralNav.includes('background:var(--paper)')&&neutralNav.includes('color:var(--muted)')&&neutralNav.includes('color:var(--ink)')&&neutralNav.includes('background:var(--fab)')&&!css.includes('.bottom-nav .nav-item:nth-child(1) .nav-item__icon'),'bottom navigation must use a neutral theme surface, muted items and the yellow Add action');
