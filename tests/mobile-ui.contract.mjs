...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v99-setup-copy'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
for(const copy of ['Your trip is taking shape.',"Still haven't booked the flights?",'Still looking for a place to stay?','Need an eSIM?','Compare routes on Aviasales','Browse stays on Booking.com','Get connected before you land','Partner links may earn Tripto a commission at no extra cost.'])assert(tripSetup.includes(copy),`post-dates trip setup missing: ${copy}`);
