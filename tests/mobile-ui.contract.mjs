...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('tripto-shell-product-v38-phosphor-icons'),'clean route, canonical host, lazy search, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, or shell cache contract changed');
...
assert(app.includes('const ICON_ALIAS = Object.freeze({')&&app.includes('const ICON_SPRITE = "/icons/tripto-system.svg?v=phosphor-v1"')&&app.includes('const FILLED_ICON_IDS = new Set(')&&app.includes('<use href="${ICON_SPRITE}#${symbol}"></use>'),'local Phosphor sprite resolver missing');
