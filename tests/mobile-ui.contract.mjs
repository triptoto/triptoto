...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('tripto-shell-product-v2-welcome-passport-v4-compact'),'clean route, canonical host, lazy search, manual-attachment, booking-email inbox, live-flight, or Google-auth shell cache contract changed');
...
assert(css.includes('.first-run-screen.welcome-thread')&&css.includes('height:100svh')&&css.includes('--welcome-title:clamp(32px,min(9.4vw,5.4svh),42px)')&&css.includes('--welcome-art-height:clamp(118px,min(45vw,21svh),190px)')&&css.includes('@media(max-height:620px)')&&css.includes('max-width:400px'),'approved compact height-aware Welcome visual system missing');
