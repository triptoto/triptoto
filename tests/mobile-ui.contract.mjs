...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('tripto-shell-product-v31-glass-nodes'),'clean route, canonical host, lazy search, manual-attachment, booking-email inbox, live-flight, Google-auth, typography, or shell cache contract changed');
...
assert(css.includes('background:var(--accent);color:var(--card)')&&css.includes('.welcome-route-matrix')&&css.includes('.welcome-route-cell--next')&&!css.includes('--welcome-serif'),'Welcome must use the selected Route Matrix visual and Apple interface typography');
