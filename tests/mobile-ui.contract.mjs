...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('tripto-shell-product-v41-manual-add'),'clean route, canonical host, lazy search, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, or shell cache contract changed');
...
for(const copy of ['Add Booking Manually','What are you adding?','Choose a type and add the confirmed details.','Already have a confirmation?','Upload a file','Forward an email'])assert(add.includes(copy),`direct manual-add page missing: ${copy}`);
