...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v87-destination-viewport'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
assert(app.includes('const ownRequest = ++request;')&&app.includes('window.setTimeout(() => search(ownRequest), 70)')&&app.includes('fullScreenPanel?.addEventListener("keydown"')&&app.includes('event.key !== "Tab"'),'Create Trip destination search must reject stale results and trap keyboard focus inside the full-screen task');
...
assert(css.includes('.full-screen-picker,.trip-create-destination.is-fullscreen{--picker-bg:var(--card);--picker-ink:var(--ink);--picker-accent:var(--accent)'),'trip pickers must share the three-color white, ink, and accent system');
