...
const index=read('public/index.html'),css=read('public/mobile-app.css'),app=read('public/mobile-app.js'),sw=read('public/sw.js'),shellUpdate=read('public/shell-update.js'),rules=read('public/mobile-trip-rules.js'),routeSource=read('public/mobile-routes.js'),manifest=read('public/manifest.webmanifest'),dmSerifCss=read('public/vendor/dm-serif-display/dm-serif-display.css'),privacy=read('public/privacy.html'),terms=read('public/terms.html'),dmSerifLatin=readFileSync('public/vendor/dm-serif-display/DMSerifDisplay-Latin.woff2'),dmSerifLatinExt=readFileSync('public/vendor/dm-serif-display/DMSerifDisplay-LatinExt.woff2');
...
assert(css.includes('--font-ui:-apple-system')&&css.includes('BlinkMacSystemFont')&&css.includes('--font-title:"DM Serif Display"')&&css.includes('--font:var(--font-ui)')&&!css.includes('Nunito')&&!css.includes('"Inter"')&&!css.includes('"Geist"'),'approved DM Serif title and Apple interface stacks changed');
assert(dmSerifCss.includes('font-family: "DM Serif Display"')&&dmSerifCss.includes('font-weight: 400')&&dmSerifCss.includes('font-display: swap')&&!/https?:\/\//.test(dmSerifCss),'DM Serif Display must be locally hosted with swap behavior');
assert(dmSerifLatin.subarray(0,4).toString()==='wOF2'&&dmSerifLatinExt.subarray(0,4).toString()==='wOF2','DM Serif Display font files are missing, corrupt, or pointer files');
assert(index.includes('rel="preload" href="/vendor/dm-serif-display/DMSerifDisplay-Latin.woff2"')&&index.indexOf('/vendor/dm-serif-display/dm-serif-display.css')<index.indexOf('/mobile-app.css')&&!index.includes('/vendor/geist/'),'DM Serif Display preload or stylesheet order changed');
assert(sw.includes('/vendor/dm-serif-display/dm-serif-display.css')&&sw.includes('/vendor/dm-serif-display/DMSerifDisplay-Latin.woff2')&&sw.includes('/vendor/dm-serif-display/DMSerifDisplay-LatinExt.woff2')&&!sw.includes('/vendor/geist/'),'DM Serif Display is not available in the offline application shell');
...
for(const selector of ['.trip-v2-selector strong','.trip-group .trip-row__copy strong','.journey-copy strong','.fd-title','.hotel-detail-screen .fd-flight','.booking-card[data-action="booking-detail"] strong','.booking-trip-group>h2','.quick-trip-context strong'])assert(css.includes(selector),`DM Serif title role missing: ${selector}`);
...
for(const color of ['--paper:#fbf8f7','--card:#ffffff','--surface:#f3eff0','--ink:#05152d','--icon:#0a2445','--accent:#5547b7','--fab:#fbc840','--green:#007852','--next:#b84d16','--flight-soft:#d9e4fb','--transfer-soft:#fee4cf','--stay-soft:#d1edde'])assert(css.includes(color),`single production palette missing: ${color}`);
...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('tripto-shell-product-v37-timeline-type-scale'),'clean route, canonical host, lazy search, manual-attachment, booking-email inbox, live-flight, Google-auth, typography, or shell cache contract changed');
...
for(const token of ['--timeline-trip-title-size:clamp(28px,7.7vw,32px)','--timeline-booking-title-size:clamp(19px,5.1vw,21px)','--timeline-metadata-size:clamp(15px,3.9vw,16px)','--timeline-status-size:clamp(14px,3.7vw,15px)','--timeline-time-size:clamp(16px,4.35vw,18px)','--timeline-day-size:clamp(14px,3.85vw,16px)'])assert(css.includes(token),`Timeline typography role missing: ${token}`);
...
for(const name of ['home','trips','flight','hotel','car','restaurant','coffee','bar','shopping','camera','tour','edit','delete','notifications','location','calendar','clock','more','close','check','chevron-right','traveler','documents','ferry','cruise','bus','taxi','confirmed','directions','mountain','beach','landmark'])assert(new RegExp(`\\n\\s{4}["']?${name}["']?:\\s*\\{ vb:`).test(reg),`approved icon missing from registry: ${name}`);}
