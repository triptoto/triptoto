...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&sw.includes('/mobile-app.min.css')&&sw.includes('/mobile-app.min.js')&&sw.includes('tripto-shell-product-v85-fullscreen-trip-pickers'),'clean route, canonical host, lazy search, optimized shell, manual-attachment, icon, booking-email inbox, live-flight, Google-auth, typography, currency, or shell cache contract changed');
...
assert(app.includes('data-place-search-close')&&app.includes('setFullScreen')&&app.includes('Search destination')&&app.includes('fullScreenPanel.focus({ preventScroll:true })')&&app.includes('request += 1;'),'Create Trip destination search must open as an accessible full-screen task, cancel stale searches, and restore focus without reopening the keyboard');
...
for(const selector of ['.trip-create-destination.is-fullscreen','.trip-create-search-guide','.trip-create-destination.is-fullscreen .place-option'])assert(css.includes(selector),`full-screen destination design missing: ${selector}`);
