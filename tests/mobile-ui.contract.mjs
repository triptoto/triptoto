...
const index=read('public/index.html'),css=read('public/mobile-app.css'),app=read('public/mobile-app.js'),sw=read('public/sw.js'),rules=read('public/mobile-trip-rules.js'),routeSource=read('public/mobile-routes.js'),manifest=read('public/manifest.webmanifest'),geistCss=read('public/vendor/geist/geist.css'),phosphorCss=read('public/vendor/phosphor/phosphor.css'),privacy=read('public/privacy.html'),terms=read('public/terms.html'),geistFont=readFileSync('public/vendor/geist/Geist-Variable.woff2'),phosphorRegular=readFileSync('public/vendor/phosphor/Phosphor.woff2'),phosphorFill=readFileSync('public/vendor/phosphor/Phosphor-Fill.woff2');
...
assert(sw.includes('/canonical-host.js')&&sw.includes('/mobile-routes.js')&&!sw.includes("'/airport-timezones.js'")&&sw.includes('/google-auth-client.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('tripto-shell-product-v2-phosphor-v39'),'clean route, canonical host, lazy search, manual-attachment, booking-email inbox, live-flight, Google-auth, Geist, or Phosphor shell cache contract changed');
...
assert(index.includes('/vendor/phosphor/phosphor.css?v=phosphor-v39')&&sw.includes('/vendor/phosphor/Phosphor.woff2')&&sw.includes('/vendor/phosphor/Phosphor-Fill.woff2'),'local Phosphor Regular/Fill library missing');
assert(phosphorRegular.subarray(0,4).toString()==='wOF2'&&phosphorFill.subarray(0,4).toString()==='wOF2','Phosphor font asset is missing or corrupt');
assert(phosphorCss.includes('font-family: "Phosphor Fill"')&&phosphorCss.includes('.nav-item.active .ph')&&phosphorCss.includes('.ph-pencil-simple::before')&&phosphorCss.includes('.ph-trash::before')&&phosphorCss.includes('.ph-bell::before'),'Phosphor selected-state or action-icon mapping missing');
assert(!app.includes('const ICONS')&&app.includes('const INLINE_SVG = Object.freeze({')&&app.includes('"wx-sun":')&&app.includes('"wx-cloud-rain":')&&app.includes('"dest-mountain":')&&app.includes('"dest-beach":')&&app.includes('"dest-monument":')&&(app.match(/<svg/g)||[]).length===1,'approved weather/destination custom SVG system missing');
for(const name of ['user','plane','hotel','car','restaurant','edit','trash','bell','home','pin','calendar','clock','more','close','check','chevron'])assert(new RegExp(`\\n\\s{4}["']?${name}["']?:\\s*\\{`).test(app.slice(app.indexOf('const INLINE_SVG'),app.indexOf('const state ='))),`custom SVG icon missing (must not fall back to Phosphor): ${name}`);
for(const mapping of ['edit: "pencil-simple"','trash: "trash"','bell: "bell"','sim: "sim-card"','external: "arrow-square-out"','bolt: "lightning"','globe: "globe"'])assert(app.includes(mapping),`Phosphor icon mapping missing: ${mapping}`);
