...
// G: address-only place resolved through the keyless geocoder (for precise directions).
assert(app.includes('function geocodeMissingTripPlaces(')&&app.includes('/api/v1/geocode?q='),'State G: address geocoding missing');
// H: offline -> no geocode attempt, list still available, directions guarded.
assert(app.includes('if (state.offline) return')&&app.includes('trip-map__offline')&&app.includes('Connect to open directions'),'State H: offline degradation missing');
...
for(const handler of ['case "open-trip-map":','case "trip-map-day":','case "trip-map-navigate":'])assert(app.includes(handler),`action handler missing: ${handler}`);
...
// --- Styling present ------------------------------------------------------
assert(css.includes('.trip-map__hero')&&css.includes('.trip-map__row')&&css.includes('.trip-map__offline')&&css.includes('.trip-map__list-head'),'Trip Map styling missing');
