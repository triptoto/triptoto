...
assert(sw.includes("const CACHE='tripto-shell-product-v33-pastel-travel'")&&sw.includes('/canonical-host.js')&&sw.includes('/manual-booking-attachments.js')&&!sw.includes('/vendor/phosphor/')&&sw.includes('/vendor/dm-serif-display/DMSerifDisplay-Latin.woff2')&&sw.includes('await cache.put(request,response.clone())'),'service worker runtime cache updated');
