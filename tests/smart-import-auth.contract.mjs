...
assert(sw.includes("const CACHE='tripto-shell-product-v225-readable-seo-routes'")&&sw.includes('/canonical-host.js')&&sw.includes('/manual-booking-attachments.js')&&sw.includes('/icons/tripto-system.svg')&&!sw.includes('/vendor/phosphor/')&&!sw.includes('/vendor/dm-serif-display/')&&sw.includes('await cache.put(request,response.clone())'),'service worker runtime cache updated');
