import { googleCredentialNonce, verifyGoogleCredential } from '../../apps/worker/src/google-auth.ts';
...
assert.equal(googleCredentialNonce(signed),'nonce-123','redirect nonce extraction');
const verified=await verifyGoogleCredential(env,signed,'nonce-123');assert.equal(verified.providerSubject,'google-subject-123');assert.equal(verified.emailVerified,true);assert.equal(verified.avatarUrl,'https://example.test/avatar.png');
await assert.rejects(async()=>googleCredentialNonce(await token({nonce:null})),/could not be verified/);
