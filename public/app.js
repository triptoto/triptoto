...
function dateRangeMarkup(startName,endName,rangeLabel,startLabel,endLabel,startValue,endValue,fieldAttribute){
  var attr=fieldAttribute||'name';
  return '<div class="field date-range-field"><label>'+esc(rangeLabel)+'</label><div class="date-range-control" data-date-range data-range-label="'+esc(rangeLabel)+'" data-start-label="'+esc(startLabel)+'" data-end-label="'+esc(endLabel)+'"><input type="hidden" '+attr+'="'+esc(startName)+'" data-range-start value="'+esc(startValue||'')+'"><input type="hidden" '+attr+'="'+esc(endName)+'" data-range-end value="'+esc(endValue||'')+'"><button type="button" class="date-range-trigger" aria-haspopup="dialog"><span><strong class="date-range-trigger__value">Choose dates</strong><small class="date-range-trigger__hint">Tap start, then end</small></span><span class="date-range-trigger__icon" aria-hidden="true">▣</span></button></div></div>';
}
    ...
    '<div class="settings-grid">'+betaLaunchCard()+
      '<section class="card card-pad"><div class="section-title"><div><div class="eyebrow">Trip</div><h2>Basics</h2></div>'+badge(t.lifecycle_state||'draft')+'</div>'+ 
        '<form id="tripSettingsForm" class="form">'+
          ...
          dateRangeMarkup('startsOn','endsOn','Trip dates','Start','End',t.starts_on,t.ends_on)+
