(function () {
  ...
  function accountScreen() {
    ...
    return `<div class="phone-app"><section class="screen mobile-v1-screen account-page">${appBar("Account")}<main class="account-section mobile-page">
      <section class="account-profile-card ds-grouped-card" aria-label="Your profile"><div class="account-profile"><span class="account-avatar" aria-hidden="true">${esc(initials)}</span><div class="account-profile__id"><h1>${esc(name)}</h1><p class="account-meta">${mode === "account" ? esc(identityEmail) : "Guest profile"}</p></div></div><div class="account-trip-summary">${tripCounts.map(({label,count}) => `<div><strong>${count}</strong><span>${label}</span></div>`).join("")}</div>${mode === "account" ? `<button type="button" class="account-signout-btn" data-action="sign-out">Sign out</button>` : ""}</section>
      ${authBlock}
