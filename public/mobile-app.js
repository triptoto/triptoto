(function () {
  ...
  function currencyScreen() {
    ...
    return `<div class="phone-app"><section class="screen currency-screen">${appBar("Currency", state.trip.title || "Trip", true)}<main class="currency-page"><section class="currency-context"><span class="currency-context__icon">${icon("currency",22)}</span><div class="currency-context__copy"><strong>${esc(destination)} · ${esc(destinationCurrency())}</strong>${status}</div><button type="button" class="currency-refresh" data-action="refresh-currency" aria-label="Update exchange rate"${state.currencyLoading ? " disabled" : ""}>${icon("refresh",20)}</button></section><section class="currency-converter" aria-labelledby="currency-converter-title"><header class="currency-card-head"><div><span>TRIP CONVERTER</span><h1 id="currency-converter-title">Know what it costs</h1></div></header><div class="currency-exchange"><section class="currency-panel currency-panel--from"><div class="currency-panel__head"><span>From</span><label><span class="sr-only">From currency</span><select data-currency-field="from" aria-label="From currency">${options(currency.from)}</select></label></div><label class="currency-amount"><span class="sr-only">Amount in ${esc(currency.from)}</span><input data-currency-amount type="number" inputmode="decimal" min="0" step="any" value="${esc(currency.amount)}" aria-label="Amount in ${esc(currency.from)}"></label><div class="currency-quick" aria-label="Quick amounts">${[10,50,100,500].map((value) => `<button type="button" data-action="currency-quick" data-value="${value}"${Number(currency.amount) === value ? " class=\"is-active\"" : ""}>${value}</button>`).join("")}</div></section><button type="button" class="currency-swap" data-action="currency-swap" aria-label="Swap currencies">${icon("swap",20)}</button><section class="currency-panel currency-panel--to"><div class="currency-panel__head"><span>To</span><label><span class="sr-only">To currency</span><select data-currency-field="to" aria-label="To currency">${options(currency.to)}</select></label></div><output class="currency-result" aria-live="polite"><strong class="currency-result__amount">${esc(result == null ? "—" : money(result, currency.to))}</strong><span>${esc(currency.to)} · ${esc(TRAVEL_CURRENCIES.find(([code]) => code === currency.to)?.[1] || "Currency")}</span></output></section></div><footer class="currency-rate-footer"><strong>${Number.isFinite(rate) ? `1 ${esc(currency.from)} = ${esc(rate.toFixed(rate < 1 ? 4 : 3))} ${esc(currency.to)}` : "Update to load this rate"}</strong><span>${esc(currency.source || "Daily reference rates")}</span></footer>${error}</section><p class="currency-disclaimer">Reference rate only; providers may add fees. Amounts are calculated on this phone.</p></main></section></div>`;
  }
  function esimScreen() {
    const dest =
