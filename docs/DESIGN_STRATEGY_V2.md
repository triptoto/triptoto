...
- The current UI uses the deterministic local sprite generated by `npm run build:icons`; no icon library or CDN is requested at runtime.
...
The Trip Map uses the production Apple system font stack, deep navy text, purple active-day/NEXT accents, and the same pastel category colors as Timeline icons. A compact pastel route summary is followed by day chips and white destination cards. Every card has a category icon, real booking time/address, and an explicit single-destination Directions action. Touch targets stay >=44px. There is no multi-point Google Maps action, embedded map, fabricated pin, or separate visual theme. See TRIP_MAP_V2.md.
