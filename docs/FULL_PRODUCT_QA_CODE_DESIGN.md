...
| QA-014 | Accessibility | P2 | Fixed | `--muted-soft` now uses accessible slate `#667180` (4.69:1 on the production paper canvas). Success, warning, and error colors were also darkened so small status text clears AA on its matching pastel surface. |
