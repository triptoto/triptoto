...
// Cloudflare Email Routing delivers the original custom address in `to`.
// Keep the accepted envelope surface exactly aligned with the public address.
const ACCEPTED_RECIPIENTS = new Set(['go@tripto.to']);
...
export async function receiveBookingEmail(message: InboundEmailMessage, env: Env): Promise<void> {
  ...
  const now = nowMs(), tripId = parsed.candidates.length && trips.length === 1 ? trips[0].id : null, importId = uuid();
  const status = !parsed.candidates.length ? 'unsupported' : tripId ? 'needs_confirmation' : 'needs_trip';
  const rejectionCode = !parsed.candidates.length ? 'UNSUPPORTED_FORMAT' : tripId ? null : trips.length ? 'AMBIGUOUS_TRIP' : 'NO_ELIGIBLE_TRIP';
  const statements = [
    env.DB.prepare(`INSERT INTO imports(id,trip_id,user_id,source_type,status,source_fingerprint,recovery_action,created_at) VALUES (?,?,?,'forwarded_email',?,?,?,?)`).bind(importId,tripId,verified.user_id,parsed.candidates.length?'needs_confirmation':'unsupported',fingerprint,parsed.unsupportedReason ?? 'Review extracted booking details before confirming.',now),
  ...
  statements.push(env.DB.prepare(`INSERT INTO inbound_booking_emails(id,user_id,trip_id,import_id,sender_normalized,message_fingerprint,subject,status,rejection_code,received_at) VALUES (?,?,?,?,?,?,?,?,?,?)`).bind(uuid(),verified.user_id,tripId,importId,sender,fingerprint,subject,status,rejectionCode,now));
