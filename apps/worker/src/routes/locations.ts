...
interface LocationBody { type?:unknown; displayName?:unknown; localName?:unknown; formattedAddress?:unknown; localAddress?:unknown; latitude?:unknown; longitude?:unknown; countryCode?:unknown; regionCode?:unknown; city?:unknown; timezone?:unknown; iataCode?:unknown; icaoCode?:unknown; stationCode?:unknown; }
...
export async function createLocation(request:Request,env:Env,auth:AuthContext,tripId:string):Promise<Response>{
  ...
  await env.DB.batch([
    env.DB.prepare(`INSERT INTO locations (id,type,display_name,local_name,formatted_address,local_address,latitude,longitude,country_code,region_code,city,timezone,iata_code,icao_code,station_code,created_at,updated_at,version) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)`).bind(id,values.type,values.displayName,values.localName,values.formattedAddress,values.localAddress,values.latitude,values.longitude,values.countryCode,values.regionCode,values.city,values.timezone,values.iataCode,values.icaoCode,values.stationCode,now,now),
...
function normalize(body:LocationBody){
  ...
  const countryCode=upper(body.countryCode,'countryCode',2); const regionCode=upper(body.regionCode,'regionCode',20);
  ...
  return {type,displayName,localName,formattedAddress,localAddress,latitude,longitude,countryCode,regionCode,city,timezone,iataCode,icaoCode,stationCode};
}
