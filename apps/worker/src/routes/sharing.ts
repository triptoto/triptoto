...
export async function listMembers(request:Request,env:Env,auth:AuthContext,tripId:string):Promise<Response>{
  ...
  const rows=(await env.DB.prepare(`SELECT tm.user_id,CASE WHEN t.owner_user_id=tm.user_id THEN 'owner' WHEN tm.role='owner' THEN 'editor' ELSE tm.role END AS role,tm.status,tm.joined_at,u.display_name
    FROM trip_members tm JOIN trips t ON t.id=tm.trip_id JOIN users u ON u.id=tm.user_id
    WHERE tm.trip_id=? AND tm.status='active' AND u.deleted_at IS NULL
    ORDER BY CASE WHEN t.owner_user_id=tm.user_id THEN 0 WHEN tm.role IN ('owner','editor') THEN 1 ELSE 2 END,tm.joined_at`).bind(tripId).all()).results??[];
...
export async function createInvite(request:Request,env:Env,auth:AuthContext,tripId:string):Promise<Response>{
  ...
  const inserted=await env.DB.prepare(`INSERT INTO trip_invites(id,trip_id,invited_email,role,token_hash,status,created_by_user_id,expires_at,created_at)
    SELECT ?,?,?,?,?,'invited',?,?,?
    WHERE (SELECT COUNT(*) FROM trip_invites WHERE trip_id=? AND status='invited' AND expires_at>?) < ?
      AND (SELECT COUNT(*) FROM trip_members WHERE trip_id=? AND status='active') < ?
      AND (? IS NULL OR NOT EXISTS (SELECT 1 FROM trip_invites WHERE trip_id=? AND lower(invited_email)=lower(?) AND status='invited' AND expires_at>?))`)
    .bind(id,tripId,email,role,tokenHash,auth.userId!,expiresAt,now,tripId,now,PRODUCT_LIMITS.pendingInvitesPerTrip,tripId,PRODUCT_LIMITS.tripMembers,email,tripId,email,now).run();
  if(Number(inserted.meta?.changes??0)!==1){
    if(email){const duplicate=await env.DB.prepare(`SELECT 1 AS found FROM trip_invites WHERE trip_id=? AND lower(invited_email)=lower(?) AND status='invited' AND expires_at>? LIMIT 1`).bind(tripId,email,now).first();if(duplicate)throw new HttpError(409,'INVITE_ALREADY_PENDING','An active invite already exists for this email address.');}
    const currentPending=await env.DB.prepare(`SELECT COUNT(*) AS count FROM trip_invites WHERE trip_id=? AND status='invited' AND expires_at>?`).bind(tripId,now).first<{count:number}>();
    if(Number(currentPending?.count??0)>=PRODUCT_LIMITS.pendingInvitesPerTrip)throw new HttpError(409,'INVITE_LIMIT_REACHED','Too many pending invites for this trip.');
    const currentMembers=await env.DB.prepare(`SELECT COUNT(*) AS count FROM trip_members WHERE trip_id=? AND status='active'`).bind(tripId).first<{count:number}>();
    if(Number(currentMembers?.count??0)>=PRODUCT_LIMITS.tripMembers)throw new HttpError(409,'MEMBER_LIMIT_REACHED','Trip member limit reached.');
    throw new HttpError(409,'INVITE_CREATE_CONFLICT','The invitation could not be created. Try again.');
  }
...
export async function acceptInvite(request:Request,env:Env,auth:AuthContext):Promise<Response>{
  ...
  const existingMember=await env.DB.prepare(`SELECT 1 AS found FROM trip_members WHERE trip_id=? AND user_id=? AND status='active' LIMIT 1`).bind(invite.trip_id,auth.userId).first();
  if(!existingMember&&Number(count?.count??0)>=PRODUCT_LIMITS.tripMembers)throw new HttpError(409,'MEMBER_LIMIT_REACHED','Trip member limit reached.');
  const acceptance=await env.DB.batch([
    env.DB.prepare(`UPDATE trip_invites SET status='accepted',accepted_by_user_id=?,accepted_at=? WHERE id=? AND status='invited' AND expires_at>?
      AND (EXISTS (SELECT 1 FROM trip_members WHERE trip_id=? AND user_id=? AND status='active') OR (SELECT COUNT(*) FROM trip_members WHERE trip_id=? AND status='active') < ?)`)
  ...
  if(Number(acceptance[0]?.meta?.changes??0)!==1||!accepted||accepted.status!=='accepted'||accepted.accepted_by_user_id!==auth.userId){
    const currentMembers=await env.DB.prepare(`SELECT COUNT(*) AS count FROM trip_members WHERE trip_id=? AND status='active'`).bind(invite.trip_id).first<{count:number}>();
...
// Owner hands ownership to an existing active member. The canonical owner row
// is compare-and-swapped first; every membership update is conditional on that
// exact version change so a losing concurrent transfer cannot grant owner role.
export async function transferOwnership(request:Request,env:Env,auth:AuthContext,tripId:string):Promise<Response>{
  ...
  const trip=await env.DB.prepare(`SELECT owner_user_id,version FROM trips WHERE id=? AND deleted_at IS NULL`).bind(tripId).first<{owner_user_id:string|null;version:number}>();
  if(!trip||trip.owner_user_id!==auth.userId)throw new HttpError(403,'OWNER_REQUIRED','Only the trip owner can manage sharing.');
  ...
  const now=nowMs(),nextVersion=Number(trip.version)+1;
  const transfer=await env.DB.batch([
    env.DB.prepare(`UPDATE trips SET owner_user_id=?,updated_at=?,version=version+1 WHERE id=? AND deleted_at IS NULL AND owner_user_id=? AND version=? AND EXISTS (SELECT 1 FROM trip_members WHERE trip_id=? AND user_id=? AND status='active')`).bind(targetUserId,now,tripId,auth.userId,trip.version,tripId,targetUserId),
    env.DB.prepare(`INSERT INTO trip_members(trip_id,user_id,role,status,joined_at)
      SELECT ?,?,'editor','active',? WHERE EXISTS (SELECT 1 FROM trips WHERE id=? AND owner_user_id=? AND version=? AND deleted_at IS NULL)
      ON CONFLICT(trip_id,user_id) DO UPDATE SET role='editor',status='active',removed_at=NULL`).bind(tripId,auth.userId,now,tripId,targetUserId,nextVersion),
    env.DB.prepare(`UPDATE trip_members SET role='editor' WHERE trip_id=? AND role='owner' AND user_id<>? AND EXISTS (SELECT 1 FROM trips WHERE id=? AND owner_user_id=? AND version=? AND deleted_at IS NULL)`).bind(tripId,targetUserId,tripId,targetUserId,nextVersion),
    env.DB.prepare(`UPDATE trip_members SET role='owner',status='active',removed_at=NULL WHERE trip_id=? AND user_id=? AND EXISTS (SELECT 1 FROM trips WHERE id=? AND owner_user_id=? AND version=? AND deleted_at IS NULL)`).bind(tripId,targetUserId,tripId,targetUserId,nextVersion),
  ]);
  const check=await env.DB.prepare(`SELECT owner_user_id FROM trips WHERE id=?`).bind(tripId).first<{owner_user_id:string}>();
  if(Number(transfer[0]?.meta?.changes??0)!==1||check?.owner_user_id!==targetUserId)throw new HttpError(409,'TRANSFER_FAILED','Ownership transfer did not complete. Try again.');
