...
interface Body{kind?:unknown;title?:unknown;status?:unknown;startsAtUtc?:unknown;endsAtUtc?:unknown;timezone?:unknown;locationId?:unknown;activityType?:unknown;reservationType?:unknown;reference?:unknown;arrivalDeadlineUtc?:unknown;windowStartUtc?:unknown;windowEndUtc?:unknown;notes?:unknown;confidence?:unknown;version?:unknown;}
...
export async function listActivities(request:Request,env:Env,auth:AuthContext,tripId:string):Promise<Response>{
  ...
  const result=await env.DB.prepare(`SELECT ti.*,a.activity_type,a.venue_location_id,a.reservation_reference,a.arrival_deadline_utc,a.notes AS activity_notes,r.reservation_type,r.confirmation_number,r.window_start_utc,r.window_end_utc,r.notes AS reservation_notes FROM trip_items ti LEFT JOIN activities a ON a.trip_item_id=ti.id LEFT JOIN reservations r ON r.trip_item_id=ti.id WHERE ti.trip_id=? AND ti.type IN ('activity','reservation') AND ti.deleted_at IS NULL ORDER BY ti.starts_at_utc,ti.created_at`).bind(tripId).all();
...
export async function createActivity(request:Request,env:Env,auth:AuthContext,tripId:string):Promise<Response>{
  ...
  await recordChangeEvent(env,tripId,'trip_item',id,`${values.kind}_added`,null,item);
