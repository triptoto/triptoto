...
export async function assignBookingEmail(request: Request, env: Env, auth: AuthContext, emailId: string): Promise<Response> {
  ...
  if (!email.import_id) throw new HttpError(409, 'BOOKING_EMAIL_NOT_REVIEWABLE', 'This email did not produce reviewable booking details.');
  ...
  if (!results.every((result) => result.success)) throw new HttpError(500, 'BOOKING_EMAIL_ASSIGN_FAILED', 'The booking email could not be assigned safely.');
