...
export async function expandedTripHealth(request:Request,env:Env,auth:AuthContext,tripId:string,persist=false):Promise<Response>{
  ...
  const connectionRows=(await env.DB.prepare(`SELECT id,from_item_id,to_item_id,connection_type,recommended_buffer_minutes,minimum_buffer_minutes,requires_airport_change,requires_baggage_reclaim FROM connections WHERE trip_id=?`).bind(tripId).all<Record<string,unknown>>()).results??[];
  ...
  const counts=await env.DB.prepare(`SELECT (SELECT COUNT(*) FROM travelers WHERE trip_id=? AND deleted_at IS NULL) traveler_count,(SELECT COUNT(*) FROM trip_items WHERE trip_id=? AND type='stay' AND deleted_at IS NULL) stay_count,(SELECT COUNT(*) FROM trip_items WHERE trip_id=? AND type='transport' AND deleted_at IS NULL) transport_count`).bind(tripId,tripId,tripId).first<{traveler_count:number;stay_count:number;transport_count:number}>();
